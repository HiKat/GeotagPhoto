...
> **Windows専用** です（ExifToolのパス処理がWindows向け）。macOS/Linuxでは動作しません。

---

## 前提条件: ExifToolのインストール（必須）

利用方法に関わらず、事前に **ExifTool** のインストールが必要です。

1. 公式サイトからWindows版のZIPファイルをダウンロード
   - https://exiftool.org/
2. ZIPファイルを展開し、中にある `exiftool(-k).exe` を **`exiftool.exe`** にリネーム
3. `exiftool.exe` を適切な場所に配置（例: `C:\Program Files\exiftool`）
...
- `GeotagPhoto.exe` - アプリケーション本体
- `README.txt` - 簡易マニュアル

#### 実行方法

1. ZIPファイルを展開
2. `GeotagPhoto.exe` をダブルクリックして起動
3. 初回起動時に `config.json` が自動生成されます

#### 注意事項

- Windows Defenderなどで「不明な発行元」の警告が出る場合があります
- 安全なアプリケーションですので、「詳細情報」→「実行」で起動してください
...
3. **Garmin Connect APIの利用制限**
   - 過度なリクエストを避けてください（1回あたり7日以内を推奨）
   - レート制限に達すると一時的にアクセスできなくなります
...
本プロジェクトは **GNU General Public License v3.0 or later (GPLv3-or-later)** の条件で配布されます。  
詳細はリポジトリルートの [`COPYING`](COPYING) を参照してください。
